  ...
  return (
    <div className={`page-container runs-page${home ? " home-page" : ""}`}>
      {home ? (
        <header className="research-entry">
          ...
          <Link className="research-composer" href="/new-run/" prefetch={false}>
            ...
            <span className="composer-cta">{ko ? "요청서 작성" : "Write the request"}</span>
          </Link>
        </header>
      ) : (
      ...
      {runs.length === 0 ? (
        ...
        <>
          {home && <div className="section-heading"><h2>{ko ? "최근 연구" : "Recent research"}</h2><Link href="/runs/">{ko ? "전체 연구 보기" : "View all runs"}</Link></div>}
