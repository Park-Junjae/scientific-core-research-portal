...
export function IdeaReader({ idea, markdownByLanguage }: { idea: ResearchIdeaManifest; markdownByLanguage: Record<string, string> }) {
  ...
  const languages = useMemo(() => Object.keys(idea.language_variants), [idea.language_variants]);
  ...
  return (
    <>
      <div className="idea-actions">
        <label className="language-select"><Languages size={17} /><span className="sr-only">Report language</span><select value={language} onChange={(event) => setLanguageOverride(event.target.value)}>{languages.map((code) => <option key={code} value={code}>{idea.language_variants[code]}</option>)}</select></label>
        {preferences.pdf === "Inline viewer" ? <button className="secondary-button" type="button" onClick={() => setView("pdf")}><ExternalLink size={17} />Open PDF</button> : <a className="secondary-button" href={withBasePath(pdf)} target="_blank" rel="noreferrer"><ExternalLink size={17} />Open PDF</a>}
