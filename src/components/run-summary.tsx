...
function variantsForLocale(reports: ResearchReportManifestV2[], locale: "ko" | "en") {
  const visible = reports.filter(isVisiblePublicReport);
  const groups = Array.from(new Set(visible.filter((report) => ["KNOWLEDGE_BACKGROUND", "IDEA_REPORT", "PORTFOLIO_DECISION", "MEASUREMENT_REPORT"].includes(report.role)).map((report) => report.translation_group_id)));
  return groups.map((group) => visible.find((report) => report.translation_group_id === group && report.language === locale)).filter((report): report is ResearchReportManifestV2 => Boolean(report)).sort((a, b) => a.display_order - b.display_order);
}
...
export function RunSummary({ run }: { run: RunWithIdeas }) {
  ...
  return <div className="summary-document">
    ...
    {reports.length > 0 && <section><h2>{t("mainReports")}</h2><div className="document-list">{reports.map((report) => <article key={report.report_id}><div><p className="document-role">{report.role.replaceAll("_", " ")}</p><h3>{localized(report.localized_title, locale)}</h3><p>{report.language.toUpperCase()}{report.page_count ? ` · ${report.page_count} ${t("pages")}` : ""}{report.reference_count ? ` · ${report.reference_count} ${t("references")}` : ""}</p></div><div className="document-actions"><Link href={`/runs/${run.slug}/reports/${report.report_id}/`}>{locale === "ko" ? "보고서 보기" : "Read report"}</Link><a href={withBasePath(report.path!)} target="_blank" rel="noreferrer"><ExternalLink size={16} />{locale === "ko" ? "파일 열기" : "Open artifact"}</a></div></article>)}</div></section>}
    <section><h2>{locale === "ko" ? "근거 상태" : "Evidence state"}</h2><LiteratureScope stats={run.literature_stats} enabled={run.literature_scope_enabled !== false} currentStage={run.status === "RUNNING" ? run.current_stage : undefined} />{run.literature_scope_enabled !== false && <Link className="text-action" href={`/runs/${run.slug}/literature/`}>{locale === "ko" ? "문헌 근거 전체 보기" : "Review all literature evidence"}</Link>}{keyLiterature.length > 0 && <SourceList runSlug={run.slug} sources={keyLiterature} compact />}</section>
