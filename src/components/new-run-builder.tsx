...
export function NewRunBuilder() {
  ...
  const labels = ko
    ? {
        ...
        references: "참고 자료와 반드시 지켜야 할 조건",
        ...
        auth: "직접 실행을 준비하려면 허용된 GitHub 계정으로 로그인해야 합니다.",
    ...
    : {
        ...
        auth: "Sign in with an allowlisted GitHub account to prepare a direct run.",
      ...
      const run = await createControlledRun(
        {
          ...
          objectives,
          constraints,
          requested_mode: value.run_type || "AUTO",
          include_literature_list_and_review_scope: false,
