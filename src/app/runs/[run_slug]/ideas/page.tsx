...
export default async function IdeasPage({ params }: { params: Promise<{ run_slug: string }> }) { const { run_slug } = await params; let run; try { run = getRun(run_slug); } catch { notFound(); } return <div className="run-page"><RunHeader run={run} /><RunTabs slug={run.slug} active="Ideas" />{run.creativity_profile === "BREAKTHROUGH_DISCOVERY" && run.breakthrough_discovery && <BreakthroughDiscoveryView data={run.breakthrough_discovery} />}<IdeasPortfolio ideas={run.ideas} runSlug={run.slug} runMode={run.run_mode} /></div>; }
